...
from det.layers import Conv2d, ShapeSpec, cat
from det.structures import Instances, heatmaps_to_keypoints

_TOTAL_SKIPPED = 0

__all__ = [
    'RoIKeypointHeadRegistry',
    'build_keypoint_head',
...
class RoIKeypointHeadRegistry(Registry):
...
def build_keypoint_head(cfg: CfgNode, input_shape: ShapeSpec) -> nn.Module:
    """Builds a keypoint head from `cfg.MODEL.ROI_KEYPOINT_HEAD.NAME`."""
    keypoint_head_name = cfg.MODEL.ROI_KEYPOINT_HEAD.NAME
    keypoint_head_cls = RoIKeypointHeadRegistry.get(keypoint_head_name)
    if hasattr(keypoint_head_cls, 'from_config'):
        keypoint_head = keypoint_head_cls.from_config(cfg, input_shape)
    else:
        keypoint_head = keypoint_head_cls(cfg, input_shape)
    return keypoint_head
...
class BaseKeypointRCNNHead(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        loss_normalizer: Union[float, str] = 1.0,
    ) -> None:
        """
        Args:
            num_keypoints: Number of keypoints to predict
            loss_weight: Weight to multiple on the keypoint loss
    ...
    @classmethod
    def from_config(cls, cfg: CfgNode, input_shape: ShapeSpec) -> 'BaseKeypointRCNNHead':
        ...
        if not normalize_by_visible:
            ...
            loss_normalizer = num_keypoints * batch_size_per_image * positive_sample_fraction
        else:
    ...
    def forward(self, x, instances: List[Instances]):
        ...
        if self.training:
            ...
            return {
                'loss_keypoint':
                    keypoint_rcnn_loss(x, instances, normalizer=normalizer) * self.loss_weight
            }
        else:
            keypoint_rcnn_inference(x, instances)
            return instances

    def layers(self, x: torch.Tensor) -> torch.Tensor:
        """Neural network layers that makes predictions from regional input features."""
        raise NotImplementedError


@RoIKeypointHeadRegistry.register('KRCNNConvDeconvUpsampleHead')
class KRCNNConvDeconvUpsampleHead(BaseKeypointRCNNHead):
    ...
    def __init__(
        self, input_shape: ShapeSpec, *, num_keypoints: int, conv_dims: List[int], **kwargs: Any
    ) -> None:
        """
        Args:
    ...
    @classmethod
    def from_config(cls, cfg: CfgNode, input_shape: ShapeSpec) -> 'KRCNNConvDeconvUpsampleHead':
        num_keypoints = cfg.MODEL.ROI_KEYPOINT_HEAD.NUM_KEYPOINTS
        normalize_by_visible = cfg.MODEL.ROI_KEYPOINT_HEAD.NORMALIZE_LOSS_BY_VISIBLE_KEYPOINTS
        if not normalize_by_visible:
            batch_size_per_image = cfg.MODEL.ROI_HEADS.BATCH_SIZE_PER_IMAGE
