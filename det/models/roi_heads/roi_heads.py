...
from det.layers import ShapeSpec, nonzero_tuple
from det.structures import Boxes, ImageList, Instances, pairwise_iou
from ..backbones.resnet import BottleneckBlock, ResNet
...
def build_roi_heads(cfg: CfgNode, input_shape: Dict[str, ShapeSpec]) -> 'ROIHeads':
    """Build ROIHeads from `cfg.MODEL.ROI_HEADS.NAME`."""
...
def select_foreground_proposals(proposals: List[Instances],
                                bg_label: int) -> Tuple[List[Instances], List[torch.Tensor]]:
    """
...
def select_proposals_with_visible_keypoints(proposals: List[Instances]) -> List[Instances]:
    ...
    for proposals_per_image in proposals:
        ...
        kp_in_box = (
            (xs >= proposal_boxes[:, :, 0]) & (xs <= proposal_boxes[:, :, 2]) &
            (ys >= proposal_boxes[:, :, 1]) & (ys <= proposal_boxes[:, :, 3])
        )
...
class ROIHeads(torch.nn.Module):
    ...
    def from_config(cls, cfg):
        kwargs = {
            ...
            'proposal_append_gt':
                cfg.MODEL.ROI_HEADS.PROPOSAL_APPEND_GT,
    ...
    @torch.no_grad()
    def label_and_sample_proposals(self, proposals: List[Instances],
...
@RoIHeadsRegistry.register('Res5ROIHeads')
...
@RoIHeadsRegistry.register('StandardROIHeads')
class StandardROIHeads(ROIHeads):
    ...
    def _forward_box(self, features: Dict[str, torch.Tensor],
                     proposals: List[Instances]) -> Union[Dict[str, torch.Tensor], List[Instances]]:
        """
