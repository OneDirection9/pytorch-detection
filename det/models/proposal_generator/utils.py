...
def find_top_rpn_proposals(
    ...
    """
    For each feature map, select the `pre_nsm_topk` highest scoring proposals, apply NMS,
    clip proposals, and remove small boxes. Return the `post_nms_topk` highest scoring proposals
    among all the feature maps for each image.

    Args:
        proposals: A list of L tensors. Tensor i has shape (N, Hi*Wi*Ai, 4). All proposals
            predictions on the feature maps.
        pred_objectness_logits: A list of L tensors. Tensors i has shape (N, Hi*Wi*A).
        image_sizes: Sizes (h, w) for each image.
        nms_thresh: IoU threshold to use for NMS.
        pre_nms_topk: Number of top k scoring proposals to keep before applying NMS. When RPN is run
            on multiple feature maps (as in FPN) this number is per feature map.
        post_nms_topk: Number of top k scoring proposals to keep after applying NMS. When RPN is run
            on multiple feature maps (as in FPN) this number is total, over all feature maps.
        min_box_size: Minimum proposal box side length in pixels (absolute units wrt input images).
        training: True if proposals are to be used in training, otherwise False. This arg exists
            only to support a legacy bug; look for the "NB: Legacy bug ..." comment.

    Returns:
        List of N Instances. The i-th Instances stores post_nms_topk object proposals for image i,
        sorted by their objectness score in descending order.
    """
    num_images = len(image_sizes)
    device = proposals[0].device

    # 1. Select top-k anchor for every level and every images
    topk_scores = []  # #lvl Tensor, each of shape N x topk
    topk_proposals = []  # #lvl Tensor, each of shape N x topk x 4
