...
class CocoDetection(base.VisionDataset):
    ...
    def __init__(self, root, ann_file, image_prefix='', min_object_area=0.,
                 use_crowd=True, infer_mode=False, transform=None):
        ...
        if not tuple(categories) == self.classes:
            raise ValueError('Incompatible category names with {}'
    ...
    def _get_items(self):
        ...
        for image_id in image_ids:
            ...
            if not self._infer_mode:
                ...
                if target is None:
                    logger.warning('{}: Skip the image since it has no valid annotations'
                                   .format(image_path))
                    continue
        ...
        logger.info('Removed {} images with no valid annotations. {} images left'
                    .format(len(image_ids) - len(items), len(items)))
